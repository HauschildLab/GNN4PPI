...
def main():
    ...
    comet_directory = os.path.join(config.config['comet_dir'], config.config['project_name'],
                                   config.config['experiment_name'])
    checkpoint_directory = os.path.join(config.config['checkpoint_dir'], config.config['project_name'],
                                        config.config['experiment_name'])
    results_inner_file = os.path.join(results_directory, 'results_inner.csv')
    results_outer_file = os.path.join(results_directory, 'results_outer.csv')
    torch_trainer_file_inner = os.path.join(results_directory, 'TorchTrainerInner')
    ...
    if not os.path.exists(comet_directory):
        ...
        os.makedirs(os.path.join(comet_directory, 'inner'), exist_ok=True)
        os.makedirs(os.path.join(comet_directory, 'outer'), exist_ok=True)
        os.makedirs(torch_trainer_file_inner, exist_ok=True)
        os.makedirs(torch_trainer_file_outer, exist_ok=True)
    ...
    search_space_inner = utils.search_space_to_tune_grid(config.search_space)
    ...
    data_split = torch.load(os.path.join(config.config['data_dir'],config.config['data_split_file']))
    outer_test_idx = data_split["outer_test_idx"]
    inner_test_idx = data_split["inner_test_idx"]
    inner_train_idx = data_split["inner_train_idx"]
    inner_val_idx = data_split["inner_val_idx"]
    outer_val_idx = data_split["outer_val_idx"]
    outer_train_idx = data_split["outer_train_idx"]

    # Configurate used computational resources
    if 'num_workers' in config.config:
    ...
    ################################ INNER SPLIT ###############################################################
    ray_trainer_inner = TorchTrainer(
        tune.with_parameters(train_func_inner, train_idx=inner_train_idx, val_idx=inner_val_idx,
                             test_idx=inner_test_idx),
        scaling_config=scaling_config
    )

    # Automated resume, if previous tuning not finished
    if tune.Tuner.can_restore(torch_trainer_file_inner):
        print('------RESUMING INNER TUNER')
        tuner_inner = tune.Tuner.restore(
            torch_trainer_file_inner,
            trainable=ray_trainer_inner,
            resume_errored=True,
            resume_unfinished=True
        )
    else:
        print('------NEW INNER TUNER')
        tuner_inner = tune.Tuner(
            ray_trainer_inner,
            param_space={"train_loop_config": search_space_inner},
            tune_config=tune.TuneConfig(
                num_samples=num_samples
            ),
            run_config=RunConfig(
                storage_path=results_directory,
                name='TorchTrainerInner',
                failure_config=FailureConfig(max_failures=10))
        )

    results_inner = tuner_inner.fit()

    # Store results of inner folds in dataframe
    df_inner = results_inner.get_dataframe()
    df_inner.to_csv(results_inner_file, index=False, sep=',')

    inner_results_df = process_predictions(df_inner, results_directory, results_inner_file, mode='inner')
    results_inner_best_df = analyze_results(inner_results_df, results_directory, mode='inner')

    if config.config['model_name'] == 'GAT' or config.config['model_name'] == 'GAT2MLP':
        results_inner_best_param_df = results_inner_best_df[['config/train_loop_config/dataset_test_round',
                                                             'config/train_loop_config/learning_rate',
                                                             'config/train_loop_config/dropout_rate_normal',
                                                             'config/train_loop_config/dropout_rate_attention',
    ...
    ################################### OUTER SPLIT ##################################################
    test_parameters = results_inner_best_param_df.to_dict('records')
    es_folds = np.arange(0, config.config['cv_fold_size_es_outer']).tolist()
    search_space_outer = {'test_params': ray.tune.grid_search(test_parameters),
                          'es_folds': ray.tune.grid_search(es_folds)}

    ray_trainer_outer = TorchTrainer(
        tune.with_parameters(train_func_outer, train_idx=outer_train_idx, val_idx=outer_val_idx,
    ...
    results_outer = tuner_outer.fit()

    # Store results of inner folds in dataframe
    df_outer = results_outer.get_dataframe()
    df_outer.to_csv(results_outer_file, index=False, sep=',')
    outer_results_df = process_predictions(df_outer, results_directory, results_outer_file, 'outer')
    analyze_results(outer_results_df, results_directory, 'outer')

    '''
    # Compress
    with tarfile.open(checkpoint_directory+'.tar.xz', "w:xz") as tar:
        tar.add(checkpoint_directory, arcname=os.path.basename(checkpoint_directory))

    # Remove directory (exists in compressed format now!)
    if os.path.exists(checkpoint_directory+'.tar.xz'):
...
def train_func_inner(hyperparameters, train_idx=None, val_idx=None, test_idx=None):
    # Seed for reproducibility (same for same inner split ->same prerequisites for different hyperparameters)
    # pl.seed_everything(42, workers=True) # used for testrun (for hyperparameter grid reduction)
    pl.seed_everything(100*hyperparameters['dataset_test_round']+hyperparameters['dataset_fold_round'])
    ...
    if 'dropout_rate' in hyperparameters:
        ...
        run_id = 'lr{0},wd{1},dropout_normal{2},dropout_att{3},hidden_layers{4},width{5},batch_size{6},test_round{7},' \
                 'fold{8}_inner'.format(str(hyperparameters['learning_rate']), str(hyperparameters['weight_decay']),
                                        str(hyperparameters['dropout_rate_normal']),
                                        str(hyperparameters['dropout_rate_attention']),
    ...
    if config.config['comet_logger']:
        if config.config['offline']:
            comet_logger = CometLogger(
                save_dir=os.path.join(config.config['comet_dir'], project_name, experiment_name, 'inner'),
    ...
    test_idx = test_idx[hyperparameters['dataset_test_round'], hyperparameters['dataset_fold_round']]
    ...
    if 'coarsening' in config.config and config.config['coarsening']:
        datamodule = KIRCDataModule(
            ...
            num_workers=config.config['num_workers_datamodule'], train_idx=train_idx, val_idx=val_idx,
            test_idx=test_idx,
...
def train_func_outer(hyperparameters, train_idx=None, val_idx=None, test_idx=None):
    # Seed for reproducibility  (same for same split ->same prerequisites for different hyperparameters)
    # pl.seed_everything(42, workers=True) # used for testrun (for hyperparameter grid reduction)
    pl.seed_everything(100*hyperparameters['test_params']['config/train_loop_config/dataset_test_round']+hyperparameters['es_folds'])
    ...
    if 'config/train_loop_config/dropout_rate' in hyperparameters['test_params']:
        run_id = 'test_round{0},lr{1},wd{2},dropout{3},batchsize{4},es_fold{5}_outer'.format(
            str(hyperparameters['test_params']['config/train_loop_config/dataset_test_round']),
            str(hyperparameters['test_params']['config/train_loop_config/learning_rate']),
            str(hyperparameters['test_params']['config/train_loop_config/dropout_rate']),
            str(hyperparameters['test_params']['config/train_loop_config/weight_decay']),
            str(hyperparameters['test_params']['config/train_loop_config/batch_size']),
            str(hyperparameters['es_folds'])
        )
    else:
        run_id = 'test_round{0},lr{1},wd{2},dropout_normal{3},dropout_attention{4},batchsize{5},es_fold{5}_outer'.format(
            str(hyperparameters['test_params']['config/train_loop_config/dataset_test_round']),
            str(hyperparameters['test_params']['config/train_loop_config/learning_rate']),
            str(hyperparameters['test_params']['config/train_loop_config/dropout_rate_normal']),
            str(hyperparameters['test_params']['config/train_loop_config/dropout_rate_attention']),
            str(hyperparameters['test_params']['config/train_loop_config/weight_decay']),
            str(hyperparameters['test_params']['config/train_loop_config/batch_size']),
            str(hyperparameters['es_folds']))
    ...
    if config.config['comet_logger']:
        if config.config['offline']:
            comet_logger = CometLogger(
                save_dir=os.path.join(config.config['comet_dir'], project_name, experiment_name, 'outer'),
                project_name=project_name, experiment_name=experiment_name + '_outer',
                offline=True, log_graph=True)
        else:
            comet_logger = CometLogger(
                save_dir=os.path.join(config.config['comet_dir'], project_name, experiment_name, 'outer'),
    ...
    # Select and clear up data indices
    test_idx = test_idx[hyperparameters['test_params']['config/train_loop_config/dataset_test_round']]
    test_idx = test_idx[test_idx >= 0]
    train_idx = train_idx[
        hyperparameters['test_params']['config/train_loop_config/dataset_test_round'], hyperparameters['es_folds']]
    train_idx = train_idx[train_idx >= 0]  # cut -1 values(used for matching max shape)
    val_idx = val_idx[
        hyperparameters['test_params']['config/train_loop_config/dataset_test_round'], hyperparameters['es_folds']]
    val_idx = val_idx[val_idx >= 0]
    ...
    if 'coarsening' in config.config and config.config['coarsening']:
        datamodule = KIRCDataModule(
            config.config['data_file'], batch_size=hyperparameters['test_params']['config/train_loop_config/batch_size'],
            num_workers=config.config['num_workers_datamodule'], train_idx=train_idx, val_idx=val_idx,
            test_idx=test_idx,
            coarsening_dir=config.config['coarsening_dir']
        )
    ...
    if config.config['model_name'] == 'MLP':
        model = MLP(
            ...
            number_hidden_layers=config.config['number_hidden_layers'],
            width=config.config['width'],
            learning_rate=hyperparameters['test_params']['config/train_loop_config/learning_rate'],
            weight_decay=hyperparameters['test_params']['config/train_loop_config/weight_decay'],
            dropout_rate=hyperparameters['test_params']['config/train_loop_config/dropout_rate'],
            residual=config.config['residual'],
            dense=config.config['dense'],
            width_hidden_reduced_dense=config.config['width_hidden_reduced_dense']
        )
        ...
        model = GAT(
            ...
            learning_rate=hyperparameters['test_params']['config/train_loop_config/learning_rate'],
            weight_decay=hyperparameters['test_params']['config/train_loop_config/weight_decay'],
            dropout_rate_normal=hyperparameters['test_params']['config/train_loop_config/dropout_rate_normal'],
            dropout_rate_attention=hyperparameters['test_params']['config/train_loop_config/dropout_rate_attention'],
            residual=config.config['residual'],
            coarsening=config.config['coarsening'] if 'coarsening' in config.config else False, # coarsening key not in all configs
            dense=config.config['dense'],
            width_hidden_reduced_dense=config.config['width_hidden_reduced_dense']
    ...
    val = trainer.validate(best_model, datamodule)
    test = trainer.test(best_model, datamodule)
    # Rename result of outer split!
    val[0]['outer_es_loss'] = val[0].pop('val_loss')
    val[0]['outer_es_accuracy'] = val[0].pop('val_accuracy')
    test[0]['test_loss'] = test[0].pop('test_loss')
    test[0]['test_accuracy'] = test[0].pop('test_accuracy')
    ...
    trial_id = session.get_trial_id()
    prediction_directory = os.path.join(config.config['result_dir'], config.config['project_name'],
                                        config.config['experiment_name'], 'outer_results', trial_id)
